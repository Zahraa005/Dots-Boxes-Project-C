...
bool isValidMove(int r1, int c1, int r2, int c2) {
    ...
    if ((r1 == r2 && abs(c1 - c2) == 1) || (c1 == c2 && abs(r1 - r2) == 1)) {
        ...
        return board[(row1 + row2) / 2][(col1 + col2) / 2] == ' ';
    }
    return false;
}
...
void placeMove(int r1, int c1, int r2, int c2) {
    if (r1 == r2) {
        board[r1 * 2][(c1 + c2) * 2 / 2] = '-';
    } else {
        board[(r1 + r2) * 2 / 2][c1 * 2] = '|';
    }
}
...
void botMove(int *r1, int *c1, int *r2, int *c2, int difficulty) {
    if (difficulty == 1) {
        ...
        for (int i = 1; i < ROWS * 2 - 1; i += 2) {
            for (int j = 1; j < COLS * 2 - 1; j += 2) {
                if (board[i][j] == ' ') {
                    int count = 0, mr = -1, mc = -1;

                    if (board[i - 1][j] != '-') { count++; mr = i - 1; mc = j; }
                    if (board[i + 1][j] != '-') { count++; mr = i + 1; mc = j; }
                    if (board[i][j - 1] != '|') { count++; mr = i; mc = j - 1; }
                    if (board[i][j + 1] != '|') { count++; mr = i; mc = j + 1; }
...
int main() {
    srand(time(NULL));  // For random bot moves everytime
    int choice;
